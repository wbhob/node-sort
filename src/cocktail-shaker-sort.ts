export function cocktailShakerSort(array: any[]) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    // we perform this check twice to prevent having to unnecessarily
    // go back in the array
    if (!swapped) break

    // bubble sort backwards through each value
    for (let j = array.length - 1; j >= 0; j--) {
      // if right value is smaller than the left, swap them
      if (array[j + 1] < array[j]) {
        let memory = array[j]
        array[j] = array[j + 1]
        array[j + 1] = memory
